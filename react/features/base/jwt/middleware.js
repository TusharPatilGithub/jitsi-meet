 ...
 *
 * Currently this method normalizes only 'avatar', 'avatarUrl', 'email' and
 * 'name' fields. Any other key/value pairs are being passed over directly to
 * the output structure even though not described by the returned type.
 *
...
function _normalizeCallerFields(user) {
    const { avatar, avatarUrl, email, name, ...otherKeys } = user;
    const caller = otherKeys;
